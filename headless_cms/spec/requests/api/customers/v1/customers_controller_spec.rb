...
RSpec.describe Api::Customers::V1::CustomersController, type: :request do

  let(:token) { double(:token => "some-token", :acceptable? => true) }
  let(:customer) { build :customer }

  context "GET /api/customers/v1/customer" do
    before do
      expect_any_instance_of(Api::Customers::V1::CustomersController).to receive(:doorkeeper_token).at_least(1).times { token }
      allow_any_instance_of(Api::Customers::V1::CustomersController).to receive_message_chain("doorkeeper_token.application.owner").and_return customer

      get "/api/customers/v1/customer"
    ...
    it "should respond with schema of customer" do
